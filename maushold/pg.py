...
async def get_ids(conn: AsyncConnection, table: str, limit: int = 1_000_000, offset: int = 0) -> list[str]:
    cur = conn.cursor()
...
async def get_pop_data(conn: AsyncConnection, table: str, id: str) -> list[PopQuery]:
    ...
    await cur.execute(f"SELECT geo_id, pop FROM {table} WHERE geo_id LIKE %s", (id,)) #type: ignore
    res = await cur.fetchall()
    await cur.close()
    return res


async def get_row_by_bbox(conn: AsyncConnection, cat: CensusCategory, geom: shapely.Polygon) -> list[GeoRefPopQuery]:
